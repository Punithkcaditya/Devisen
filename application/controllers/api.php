...
class Api extends CI_Controller {
		...
		if($res){
            ...
            if ($this->form_validation->run() == true) {
                $deviceId=$this->input->post('deviceId');
                    ...
                    $msc = microtime(true);
                    $data_log_mysql="-----------------Initiated Save data call : -----------------\n";
                    file_put_contents(LOG_UPLOAD_PATH."execution_log.txt", $deviceId, FILE_APPEND);
    ...
    public function calender() {		
		...
		if($res){
            ...
            $today = $this->master_model->getTodaycalender();
            ...
            $tomorrow = $this->master_model->getTomcalender();
            $data['tomorrow'] = array();
            if(!empty($tomorrow))
                $data['tomorrow'] = $tomorrow;
    
            $this_week = $this->master_model->getThisWeekcalender();
            $data['this_week'] = array();
            if(!empty($this_week))
                $data['this_week'] = $this_week;

            
            $next_week = $this->master_model->getNextWeekcalender();
	...
	public function research() {		
		...
		if($res){
            ...
            if(!empty($daily_report)){
                foreach($daily_report as $key=>$daily){
            ...
            if(!empty($weekly_report)){
                foreach($weekly_report as $key=>$weekly){
            ...
            if(!empty($special_report)){
                foreach($special_report as $key=>$special){
    ...
    public function calculator(){
        ...
        if($res) {
			...
			file_put_contents(LOG_UPLOAD_PATH."calculator.txt",print_r($_POST,true),FILE_APPEND);
	...
	public function countrytime() {
                ...
                if($res){
                        ...
                        $Dubai = $dubai->format('H:i:s');

                        $newyork = new DateTime('now', new DateTimeZone('America/Detroit'));
                        $New_York = $newyork->format('H:i:s');

                        $london = new DateTime('now', new DateTimeZone('Europe/London'));
                        $London = $london->format('H:i:s');

                        $hongkong = new DateTime('now', new DateTimeZone('Asia/Hong_Kong'));
                        $hong_kong = $hongkong->format('H:i:s');

                        $kolkata = new DateTime('now', new DateTimeZone('Asia/Kolkata'));
                        $Kolkata = $kolkata->format('H:i:s');

                        $json_result['dubai'] = $Dubai;
                        $json_result['new_york'] = $New_York;
                        $json_result['london'] = $London;
                        $json_result['hong_kong'] = $hong_kong;
                        $json_result['kolkata'] = $Kolkata;
	...
	public function spotdate() {
		...
		if($res){
		   $d = new DateTime( date("Y-m-d") );
			...
			$json_result['spot_date'] = $d->format( 'Y-m-d' );
